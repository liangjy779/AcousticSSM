...
from utils import *
from models.s4_ss.S4_SS6 import S4Model_SS6
from models.s4_ss.S4_SS5_BIGS import S4Model_SS5_BIGS
from models.s4_ss.S4_SS5_aBIGS import S4Model_SS5_aBIGS
from copy import deepcopy
import wandb
...
class Trainer:
    ...
    def get_item(self, batchidx):
        ...
        for i in range(self.args.batch_size):
            ...
            for f in func:
                sound = f(sound)
            aug,aug1 = select_possible_augs()
            sound1 = sound.reshape(1,1,len(sound))
            sound2 = sound.reshape(1,1,len(sound))
            sound1 = torch.from_numpy(sound1)
            sound1 = sound1.float()
            sound2 = torch.from_numpy(sound2)
            sound2 = sound2.float()
            ...
            sound1 = sound1.squeeze(0).T#shape[30250,1]
            sound2 = sound2.squeeze(0).T#shape[30250,1]
    ...
    def get_item_t(self, batchidx):
        ...
        for i in range(self.args.batch_size):
            ...
            for f in func:
                ...
                sound2 = f(sound2)
            aug,aug1 = select_possible_augs()
            sound1 = sound1.reshape(1,1,len(sound1))
            sound2 = sound2.reshape(1,1,len(sound2))
            sound1 = torch.from_numpy(sound1)
            sound1 = sound1.float()
            sound2 = torch.from_numpy(sound2)
            sound2 = sound2.float()
    ...
    def train(self, epoch):
        ...
        for batch_idx in tqdm(range(self.args.train_steps)):
            ...
            cont_output = self.model(masked_inputs)
