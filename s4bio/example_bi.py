...
from models.s4.aBiGS import aBiGSLayer
...
parser.add_argument('--epochs', default=80, type=float, help='Training epochs')
# Dataset
parser.add_argument('--dataset', default='esc50', choices=['mnist', 'cifar10','speechcommand','esc50','testesc50'], type=str, help='Dataset')
...
if args.dataset == 'cifar10':
    ...
    dataset = np.load('/home/jlia/bioacoustics/S4Bioacoustics/s4bio/wav20.npz',allow_pickle=True);
    train_sounds= []
    train_labels=[]
    for i in range(1,6):
   ...
   #需要解决------------------ trainset,_ = split_train_val(speechdata,val_split=0.1)
    ...
    dataset = np.load('/home/ilee0022/bioacoustics/S4acoustic/s4bio/wav20few.npz',allow_pickle=True)
    train_sounds = []
    train_labels = []
    test_sounds = []
    test_labels = []
    ...
    testset, _ = split_train_val(test_da,val_split=0.0)
    d_input = 1
    d_output = 5
...
# sup_trainloader = torch.utils.data.DataLoader(
#     sup_train, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers)
    ...
    def __init__(
        ...
        for _ in range(n_layers):
            self.s4_layers.append(
                ...
                BiGSLayer(d_model,dropout=dropout,transposed=True,lr=min(0.001,args.lr))
            )
...
def train():
    ...
    correct = 0
    total = 0
    # pbar = tqdm(enumerate(zip(trainloader, sup_trainloader)))
    pbar = tqdm(enumerate(trainloader))
    #for batch_idx, ((inputs, targets), (sup_inputs, _)) in pbar:
    for batch_idx, (inputs, targets) in pbar:
        ...
        # mmd = mmd_loss(sup_outputs.sum(dim=0),outputs.sum(dim=0))
        # loss = loss + 0.1 * mmd
        loss.backward()
        optimizer.step()
