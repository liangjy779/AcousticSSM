...
class S4Model_SS5_aBIGS(nn.Module):
    ...
    def __init__(
        ...
        for _ in range(n_layers):
            self.s4_layers.append(
                ...
                aBiGSLayer(d_model,dropout=dropout,transposed=True,lr=min(0.001,lr))
            )
...
class S4Model_FT5_aBIGS(nn.Module):
    ...
    def __init__(
        ...
        self.ss = S4Model_SS5_aBIGS(d_input, pt=None, lr=lr, d_output=d_output, d_model=d_model, n_layers=n_layers, dropout=dropout, prenorm=prenorm, dropout_fn=dropout_fn)
