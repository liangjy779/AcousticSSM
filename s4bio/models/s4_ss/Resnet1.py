...
class ResNet1(nn.Module):
    ...
    def __init__(self, block=BasicBlock, layers=[2,2,2,2], dims=2, out_dim=512, in_channels=1, zero_init_residual=False,
    ...
    def _make_layer(self,block,planes,blocks,dims,stride=1,dilate=False):
        ...
        # for _ in range(1,blocks):
        #     layers.append(S4DBlock(d_model, lr=0.001,dropout=0.2,dropout_fn=nn.Dropout, transposed=True))
...
class S4Model_Resnet(nn.Module):
    ...
    def __init__(
        ...
        #self.resnet = ResNet1(block=BasicBlock,layers=[2,2,2,2],dims=2,out_dim=512,in_channels=in_channels)
    ...
    def forward(self,x):
        ...
        if self.pt:
            x2 = x.mean(dim=1)#(B,L)(16,512)
            x_cont = self.contlayer1(x2)#(16,512)->(16,512)
            x_cont = self.contlayer2(x_cont)
...
class S4Model_FTResnet(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.prenorm = prenorm
        self.ss = S4Model_Resnet(d_input, pt=None, lr=lr, d_output=d_output, d_model=d_model, n_layers=n_layers, dropout=dropout, prenorm=prenorm, dropout_fn=dropout_fn,in_channels=3)
